...
# OUTPUT_PATH="/data/iv/iv_prep_data"                    # Python script to run after extraction
...
if [ ${run_stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
...
if [ ${run_stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
...
if [ ${run_stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
