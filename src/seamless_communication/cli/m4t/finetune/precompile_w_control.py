...
def parse_args():
    ...
    parser.add_argument("--suffix", type=str, help="Suffix for the output file", default=None)
...
with open(manifest_path) as fp_in:
    for line in fp_in:
        try:
            ...
            lines_count += 1
            if lines_count >= max_json_lines:
                break
        except:
            continue
ds = Dataset.from_list(alljsonlines).filter(simple_text_len_filter, num_proc=64)
ds = ds.map(lambda sample: collect_fbank_and_tokens(sample, text_tokenizer, text_encoders_per_lang, feature_extractor), num_proc=64)
...
if args.suffix is None:
    ds.save_to_disk(Path(manifest_path).parent / f"precompiled/{args.split}", max_shard_size="1GB")
else:
    ds.save_to_disk(Path(manifest_path).parent / f"precompiled/{args.split}_{args.suffix}", max_shard_size="1GB")